...
add_action('wp_ajax_consultar_pontos_qc', 'handle_consultar_pontos_qc');
add_action('wp_ajax_nopriv_consultar_pontos_qc', 'handle_consultar_pontos_qc');


function handle_consultar_pontos_qc() {
    $cpf = isset($_POST['cpf']) ? sanitize_text_field($_POST['cpf']) : '';

    if (empty($cpf)) {
        wp_send_json_error('CPF não informado');
    }

    $dados = consultar_pontos_qc($cpf);

    
    wp_send_json(json_decode($dados, true));
}

function consultar_pontos_qc($cpf) {
    $url = "https://gx.befly.com.br/bsi/rest/wsPontosQc";
